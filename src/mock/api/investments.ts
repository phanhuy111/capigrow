import { mockInvestmentCategories } from "@/mock/data/investments";
...
export const mockInvestmentApi = {
  getInvestments: async (params?: {
    ...
    if (params?.riskLevel) {
      filteredInvestments = filteredInvestments.filter((inv) => inv.riskLevel === params.riskLevel);
    }

    if (params?.status) {
      filteredInvestments = filteredInvestments.filter((inv) => inv.status === params.status);
    }
